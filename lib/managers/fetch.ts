  ...
  public async fetchBalance(targetUser: string, type?: EconomyMethodOption) {
    ...
    if (type === "bank") {
      if (this.db.config.debug)
        Logger.warn(
          `[fetchManager] Bank Balance found for ${targetUser} with ${ff.bank}`
        );
      return { bank: ff.bank };
    } else if (type === "wallet") {
      if (this.db.config.debug)
        Logger.warn(
          `[fetchManager] Wallet Balance found for ${targetUser} with ${ff.wallet}`
        );
      return { wallet: ff.wallet };
    }
  }
  ...
  public async fetchCoolDowns(
    ...
    switch (coolDownOption) {
      case "daily":
        if (this.db.config.debug)
          Logger.warn(
            `[fetchManager] Found Cool Down found for ${targetUser} as ${u.daily.dailyTimeout}`
          );
        return {
      ...
      case "weekly":
        if (this.db.config.debug)
          Logger.warn(
            `[fetchManager] Found Cool Down found for ${targetUser} as ${u.weekly.weeklyTimeout}`
          );
        return {
      ...
      case "monthly":
        if (this.db.config.debug)
          Logger.warn(
            `[fetchManager] Found Cool Down found for ${targetUser} as ${u.monthly.monthlyTimeout}`
          );
        return {
